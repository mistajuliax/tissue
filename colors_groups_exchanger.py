...
class vertex_colors_to_vertex_groups(bpy.types.Operator):  
    ...
    def execute(self, context):
        ...
        if (self.red and boolCol):
            ...
            id_red = id
            obj.vertex_groups[id_red].name = f'{col_name}_red'
            id+=1
        if (self.green and boolCol):
            bpy.ops.object.vertex_group_add()
            bpy.ops.object.vertex_group_assign()
            id_green = id
            obj.vertex_groups[id_green].name = f'{col_name}_green'
            id+=1
        if (self.blue and boolCol):
            bpy.ops.object.vertex_group_add()
            bpy.ops.object.vertex_group_assign()
            id_blue = id
            obj.vertex_groups[id_blue].name = f'{col_name}_blue'
            id+=1
        if (self.value and boolCol):
            bpy.ops.object.vertex_group_add()
            bpy.ops.object.vertex_group_assign()
            id_value = id
            obj.vertex_groups[id_value].name = f'{col_name}_value'
            id+=1
...
class vertex_group_to_vertex_colors(bpy.types.Operator):  
    ...
    def execute(self, context):
        ...
        if self.channel == 'Blue':
            colors_name += "_blue"
        elif self.channel == 'False Colors':
        ...
        for f in obj.data.polygons:
            for v in f.vertices:
                ...
                v_colors[i].color = (
                    (0, 0, 1) if (self.channel == 'False Colors') else (0, 0, 0)
                )
